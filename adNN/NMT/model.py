...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query_sequences, key_value_sequences, key_value_sequence_lengths, device):
        """
        Forward prop.
        ...
        # if query_sequence_pad_length == 0:
        #     print()
        # else:
        #     print()
...
class Encoder(nn.Module):
    ...
    def forward(self, encoder_sequences, encoder_sequence_lengths, device):
        """
        Forward prop.
        ...
        encoder_sequences = self.embedding(encoder_sequences) * math.sqrt(self.d_model) \
                            + self.positional_encoding[:, :pad_length, :].to(device)  # (N, pad_length, d_model)
        ...
        for encoder_layer in self.encoder_layers:
            ...
            encoder_sequences = encoder_layer[0](query_sequences=encoder_sequences,
                                                 ...
                                                 device=device)  # (N, pad_length, d_model)
            encoder_sequences = encoder_layer[1](sequences=encoder_sequences, device=device)  # (N, pad_length, d_model)
    ...
    def forward_vec(self, encoder_embed, encoder_sequence_lengths, device):
        pad_length = encoder_embed.size(1)  # pad-length of this batch only, varies across batches

        encoder_sequences = encoder_embed * math.sqrt(self.d_model) \
                            + self.positional_encoding[:, :pad_length, :].to(device)  # (N, pad_length, d_model)
...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_sequences, decoder_sequence_lengths, encoder_sequences, encoder_sequence_lengths, device):
        """
        Forward prop.
...
class Transformer(nn.Module):
    ...
    def forward(self, encoder_sequences, decoder_sequences, encoder_sequence_lengths, decoder_sequence_lengths, device):
        """
        Forward propagation.
        ...
        encoder_sequences = self.encoder(
            ...
            device)  # (N, encoder_sequence_pad_length, d_model)

        # Decoder
        decoder_sequences = self.decoder(
            decoder_sequences, decoder_sequence_lengths, encoder_sequences,
            encoder_sequence_lengths, device)  # (N, decoder_sequence_pad_length, vocab_size)
...
class LabelSmoothedCE(torch.nn.Module):
    ...
    def forward(self, inputs, targets, lengths, device):
        """
        Forward prop.
