...
class InternalClassifier(nn.Module):
    def __init__(self, input_size, output_channels, num_classes, alpha=0.5):
        ...
        if red_kernel_size == -1:
            ...
            # self.max_pool = nn.AdaptiveMaxPool2d(red_input_size)
            # self.avg_pool = nn.AdaptiveAvgPool2d(red_input_size)

            self.linear = nn.Linear(output_channels*input_size*input_size, num_classes)
            self.forward = self.forward_wo_pooling
        else:
            self.red_input_size = int(input_size/red_kernel_size)
            # self.max_pool = nn.MaxPool2d(kernel_size=red_kernel_size)
            # self.avg_pool = nn.AvgPool2d(kernel_size=red_kernel_size)
            self.max_pool = nn.AdaptiveMaxPool2d(self.red_input_size)
            self.avg_pool = nn.AdaptiveAvgPool2d(self.red_input_size)

            self.alpha = nn.Parameter(torch.rand(1))
            self.linear = nn.Linear(output_channels*self.red_input_size*self.red_input_size, num_classes)
