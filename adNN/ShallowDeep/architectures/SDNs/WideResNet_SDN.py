...
class wide_basic(nn.Module):
    ...
    def only_forward(self, x):
        ...
        fwd = fwd + self.layers[1](x)
        # return fwd, 0, None
        return fwd, torch.tensor(0), torch.zeros([len(x), self.num_classes])
...
class WideResNet_SDN(nn.Module):
    def __init__(self, params):
        ...
        if self.input_size ==  32: # cifar10 and cifar100
            ...
            self.cur_input_size = int(self.input_size/8)
            self.init_conv = nn.Conv2d(3, self.in_channels, kernel_size=3, stride=2, padding=1, bias=True)
        ...
        end_layers.append(nn.AdaptiveAvgPool2d(output_size=4))
        end_layers.append(Flatten())
        end_layers.append(nn.Linear(256*16, self.num_classes))
