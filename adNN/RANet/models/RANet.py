...
class RAFirstLayer(nn.Module):
    def __init__(self, nIn, nOut, args):
        ...
        if args.data.startswith('cifar'):
            self.layers.append(
                ConvBasic(nIn, nOut * _grFactor[0],
...
class RANet(nn.Module):
    ...
    def adaptive_forward(self, x, threshold):
        inp = self.FirstLayer(x)
        res, low_ftrs = [], []
        classifier_idx = 0
        for ii in range(self.nScales):
            ...
            for i in range(self.nBlocks[ii + 1]):
                ...
                if n_block_curr > self.nBlocks[ii]:
                    logits = self.classifier[classifier_idx](_x)
                    probs = nn.functional.softmax(logits, dim=-1)
                    if probs.max() > threshold[classifier_idx]:
                        return res
                    res.append(logits)
                    classifier_idx += 1
            low_ftrs = _low_ftrs
    ...
    def adaptive_forward_compile(self, x):
        inp = self.FirstLayer(x)
        res, low_ftrs = [], []
        classifier_idx = torch.tensor(0)
        for ii in range(self.nScales):
            ii = torch.tensor(ii, dtype=torch.int32)
            _x = inp[ii]
            _low_ftrs = []
            n_block_curr = torch.tensor(0)
            for i in range(self.nBlocks[ii + 1]):
