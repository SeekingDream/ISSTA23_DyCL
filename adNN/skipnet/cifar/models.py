...
class FeedforwardGateI(nn.Module):
    ...
    def forward(self, x):
        ...
        softmax = softmax.reshape([-1, 2])

        return softmax, logprob
...
class ResNetFeedForwardSP(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10,
        ...
        for m in self.modules():
            ...
            self.attr_layers = {}
            self.attr_layers['group1_layer0'] = getattr(self, 'group1_layer0')
            self.attr_layers['group1_gate0'] = getattr(self, 'group1_gate0')

            for g in range(3):
                for i in range(0 + int(g == 0), self.num_layers[g]):
                    if getattr(self, 'group{}_ds{}'.format(g + 1, i)) is not None:
                        self.attr_layers['group{}_ds{}'.format(g + 1, i)] = getattr(self, 'group{}_ds{}'.format(g + 1, i))
                    self.attr_layers['group{}_layer{}'.format(g + 1, i)] = getattr(self, 'group{}_layer{}'.format(g + 1, i))
                    self.attr_layers['group{}_gate{}'.format(g + 1, i)] = getattr(self, 'group{}_gate{}'.format(g + 1, i))
    ...
    def forward(self, x):
        ...
        softmax, gprob = getattr(self, 'group1_gate0')(x)
        gprobs.append(gprob)
        if softmax[:, 1] > 0.5:
            mask = torch.ones([softmax.size(0), 1, 1, 1])
        else:
            mask = torch.zeros([softmax.size(0), 1, 1, 1])
        ...
        for g in range(3):
            for i in range(0 + int(g == 0), self.num_layers[g]):
                ...
                prev = x
                softmax, gprob = getattr(self, 'group{}_gate{}'.format(g+1, i))(x)
                if softmax[:, 1] > 0.5:
                    mask = torch.ones([softmax.size(0), 1, 1, 1])
                else:
                    mask = torch.zeros([softmax.size(0), 1, 1, 1])
    ...
    def adaptive_forward_compile(self, x):
        ...
        for g in range(3):
            for i in range(self.num_layers[g]):
                ...
                # if getattr(self, 'group{}_ds{}'.format(g + 1, i)) is not None:
                #     prev = getattr(self, 'group{}_ds{}'.format(g + 1, i))(prev)
                if 'group{}_ds{}'.format(int(g + 1), int(i)) in self.attr_layers:
...
# FeeforwardGate-I
# For CIFAR-10
def cifar10_feedforward_38(data_root, pretrained=False, **kwargs):
    """SkipNet-38 with FFGate-I"""
    model = ResNetFeedForwardSP(BasicBlock, [6, 6, 6], gate_type='ffgate1')
    test_loader = load_cifar10(data_root)
...
class RNNGatePolicy(nn.Module):
    ...
    def init_hidden(self, batch_size):
        # The axes semantics are (num_layers, minibatch_size, hidden_dim)
        init_hidden = \
            torch.zeros([1, batch_size, self.hidden_dim], requires_grad=False), \
            torch.zeros([1, batch_size, self.hidden_dim], requires_grad=False)

            # (autograd.Variable(torch.zeros(1, batch_size,
            #                                   self.hidden_dim)),
            #     autograd.Variable(
            #         torch.zeros(1, batch_size,self.hidden_dim), require))
    ...
    def forward(self, x, hidden):
        ...
        self.rnn.flatten_parameters()
        # x = x.to(self.device)
        hidden = tuple(t.to(x.device) for t in hidden)
        out, hidden = self.rnn(x.view(1, batch_size, -1), hidden)
        ...
        if self.training:
            ...
            proj = self.proj(out.squeeze())
            prob = self.prob(proj)
            prob = prob.reshape([len(out), -1])
            bi_prob = torch.cat([1 - prob, prob], dim=1)
            action = (prob > 0.5).float()
            self.saved_actions.append(action)
        action = action.view(action.size(0), 1, 1, 1).float()
        return action, bi_prob, hidden[0], hidden[1]
...
class ResNetRecurrentGateRL(nn.Module):
        ...
        for g in range(3):
            for i in range(self.num_layers[g]):
                if g == 0 and i < self.num_layers[g] - 1:
                    i = i + 1
                # if getattr(self, 'group{}_ds{}'.format(g+1, i)) is not None:
                if 'group{}_ds{}'.format(g+1, i) in self.attr_layers:
                    prev = self.attr_layers['group{}_ds{}'.format(int(g+1), int(i))](prev)
    ...
    def static_adaptive_forward_compile(self, x):
        batch_size = x.size(0)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
# for CIFAR-10
def cifar10_rnn_gate_rl_38(data_root, pretrained=False, **kwargs):
