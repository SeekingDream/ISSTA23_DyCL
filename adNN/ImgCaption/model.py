...
class Encoder(nn.Module):
    def __init__(self, encoder_type, encoded_image_size=14):
        ...
        if encoder_type == 'resnet101':
            ...
            modules = list(list(resnet.children())[0])
        elif encoder_type == 'googlenet':
            resnet = torchvision.models.googlenet(pretrained=True)
            modules = list(resnet.children())[:-3]
        elif encoder_type == 'resnext':
        ...
        self.adaptive_pool = nn.AvgPool2d((4, 4))
...
def get_decoder(decoder_type, attention_dim, embed_dim, decoder_dim, vocab_size, device, encoder_dim, dropout=0.5):
    if decoder_type == 'lstm':
        decoder = LSTMDecoderWithAttention(attention_dim, embed_dim, decoder_dim, vocab_size, device,
                                                encoder_dim, dropout)
    elif decoder_type == 'gru':
        decoder = GRUDecoderWithAttention(attention_dim, embed_dim, decoder_dim, vocab_size, device,
                                               encoder_dim, dropout)
    else:
        decoder = None
        raise NotImplemented
